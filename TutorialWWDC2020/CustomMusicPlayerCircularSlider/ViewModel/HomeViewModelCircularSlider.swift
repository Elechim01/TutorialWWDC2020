//
//  HomeViewModelCircularSlider.swift
//  TutorialWWDC2020
//
//  Created by Michele Manniello on 02/11/21.
//

import SwiftUI
import AVKit

let urlc = URL(fileURLWithPath: Bundle.main.path(forResource: "audio", ofType: "mp3")!)
class HomeViewModelCircularSlider: ObservableObject {
    
    @Published var player = try! AVAudioPlayer(contentsOf: urlc)
    
    @Published var isPlaing = false
    
    @Published var album = AlbumCustomMiusicPlayer()
...
//      12.5 = 25 => Circle Radius..
        ...
        let radiants = atan2(vector.dy - 12.5, vector.dx - 12.5)
        let tempAngle = radiants * 180 / .pi
...
//        total 180
        if value.location.x >= 0 && value.location.x <= UIScreen.main.bounds.width - 80{
            withAnimation(Animation.linear(duration: 0.1)){
